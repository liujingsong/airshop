...
App({
  onLaunch: function () {
    ...
    //  获取商城名称
    wx.request({
      url: that.globalData.mainDomain+ that.globalData.subDomain +'/config/get-value',
      data: {
        key: 'mallName'
      },
    ...
    wx.request({
      url: that.globalData.mainDomain + that.globalData.subDomain + '/score/send/rule',
      data: {
        code: 'goodReputation'
      },
      success: function (res) {
    ...
    // 获取砍价设置
    wx.request({
      url: that.globalData.mainDomain + that.globalData.subDomain + '/shop/goods/kanjia/list',
      data: {},
      success: function (res) {
        if (res.data.code == 0) {
    ...
    wx.request({
      url: that.globalData.mainDomain + that.globalData.subDomain + '/user/check-token',
      data: {
        token: token
      },
      success: function (res) {
  ...
  sendTempleMsg: function (orderId, trigger, template_id, form_id, page, postJsonString){
    var that = this;
    wx.request({
      url: that.globalData.mainDomain + that.globalData.subDomain + '/template-msg/put',
      method:'POST',
      header: {
